...
def load_music_data():
    DATA_FILENAME = Path(_file_).parent / 'data/data_project_final_2.csv'
...
# Create tabs
tabs = st.tabs(["Overview", "Los Bassanos", "Platform Correlation","Artist popularirt trend"])
#####BASSAN####
# Tab 1: Overview
with tabs[0]:
    st.header("Overview")
...
with tabs[2]:
    ...
    k = st.slider('Select number of top songs',
                ...
                value=50,
                step=10)
    # Ensure initial selections are different
    initial_x_selection = 'TikTok.Views'
    initial_y_selection = 'Spotify.Streams'

    # Create radio buttons in two separate rows
    x_platform = st.radio("Select Platform", important_platforms, index=important_platforms.index(initial_x_selection))
    ...
    y_platform = st.radio("Select Platform to Compare", y_platform_options, index=y_platform_options.index(initial_y_selection) if initial_y_selection in y_platform_options else 0)

    # Check that both selections are unique and valid
    if x_platform == y_platform:
        st.warning("Please select two different platforms.")
    else:
        st.write(f"Selected platforms for comparison: {x_platform} and {y_platform}")   # x_platform = st.selectbox("Select Platform ", important_platforms, index=important_platforms.index(sort_platform))
        # y_platform = st.selectbox("Select Platform to ompare", important_platforms, index=(important_platforms.index(sort_platform) + 1) % len(important_platforms))
